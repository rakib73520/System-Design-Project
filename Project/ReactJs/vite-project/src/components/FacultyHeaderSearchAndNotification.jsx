...
const FacultyHeaderSearchAndNotification = ({ bellClick, searchClick }) => {
  ...
  const handledeleteall = async() =>{
    try {
      await axios.get(`${domain}/api/notificationdeleteallfaculty/${facultydata.email}/${"faculty"}/`);
      setAction(action+1);
    } catch (error) {
      console.log("Error connecting to the backend!");
    }
...
export default FacultyHeaderSearchAndNotification;
